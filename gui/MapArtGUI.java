...
public class MapArtGUI {
    ...
    public static void open(Player player, ItemStack mapItem) {
        ...
        boolean nameVisible = mapItem.getItemMeta().hasDisplayName();
        gui.setItem(29, createItem(nameVisible ? Material.LIME_STAINED_GLASS_PANE : Material.RED_STAINED_GLASS_PANE,
                "§eToggle Map Name", "§7Click to " + (nameVisible ? "§cHide" : "§aShow") + " the map name"));

        boolean hologramVisible = mapItem.getItemMeta().getPersistentDataContainer()
                .getOrDefault(LockUtil.LOCK_KEY, PersistentDataType.BYTE, (byte) 1) == 1;
        gui.setItem(33, createItem(hologramVisible ? Material.LIME_STAINED_GLASS_PANE : Material.RED_STAINED_GLASS_PANE,
                "§eToggle Creator Hologram", "§7Click to " + (hologramVisible ? "§cHide" : "§aShow") + " creator tag below frame"));
